...
describe 'Hoverfly Control Gem' do
  context 'used as a proxy' do
    it "doesn\'t die with concurrency" do
      Hoverfly.start('test1', 'proxy')
      Hoverfly.import(['./spec/test_simulations/middleware.json'])
      rd, wr = IO.pipe
      Process.fork do
        rd.close
        Hoverfly.start('test2', 'proxy', admin: 8887, proxy: 8499)
  ...
  context 'used as a webserver' do
    it "doesn\'t die with concurrency" do
      ...
      Process.fork do
        ...
        Hoverfly.import(['./spec/test_simulations/test.json'])
        fork_response = `curl http://localhost:8499/test`
        wr.write fork_response
        wr.close
        Hoverfly.stop
      end
      wr.close
      Process.wait
      final_response = `curl http://localhost:8500`
      final_response << "\n&&\n" << rd.read
      expect(final_response).to eq("You have successfully hit the middleware\n&&\nYou have successfully imported this simulation")
      Hoverfly.stop
    end
    ...
    it 'successfully sets a middleware' do
      ...
      Process.fork do
        ...
        Hoverfly.import(['./spec/test_simulations/test.json'])
        Hoverfly.middleware(remote: 'http:localhost:8500/test')
        response = `curl http://localhost:8500`
        wr.write response
        wr.close
        Hoverfly.stop
      end
